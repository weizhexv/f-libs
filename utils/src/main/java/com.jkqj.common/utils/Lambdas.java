...
public final class Lambdas {
    ...
    public static <T, K, V> Map<K, V> extract2Map(Collection<T> entities,
        ...
        return stream(entities)
                .filter(entity -> keyFunction.apply(entity) != null)
                .filter(entity -> valueFunction.apply(entity) != null)
                .collect(toMap(keyFunction, valueFunction, (x, y) -> x));
    }
     ...
     * @return
     */
    public static <T, U extends Comparable<? super U>> U max(Collection<T> entities,
                                                             Function<? super T, ? extends U> function) {
        if (isEmpty(entities)) {
            return null;
        }

        return stream(entities).sorted(comparing(function).reversed()).map(function).findFirst().orElse(null);
    }
